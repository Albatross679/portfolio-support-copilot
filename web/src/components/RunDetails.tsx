    ...
    <section className="run-details" aria-label="Run details">
      ...
      <div className="detail-grid">
        <section>
          ...
          <dl>
            ...
            <div><dt>Format</dt><dd>{value(run.extraction?.format)}</dd></div>
        ...
        <section>
          <h2>Chosen route</h2>
          <p className="route">{run.route ?? "Pending routing"}</p>
      ...
      {run.status === "awaiting_approval" && run.proposed_refund && (
        <section className="approval-card" aria-label="Awaiting approval">
          ...
          <p className="refund-amount">{new Intl.NumberFormat("en-US", { style: "currency", currency: run.proposed_refund.currency }).format(run.proposed_refund.amount)}</p>
