...
function valuesFor(row: Row): FormValues {
  return Object.fromEntries(Object.entries(row).filter(([key]) => key !== "id").map(([key, value]) => [key, key === "ordered_at" ? localDateTimeValue(value) : String(value)]));
}
...
export function BusinessDataView({ client = api }: BusinessDataViewProps) {
  ...
  const selectedTable = useRef(table);
  ...
  const load = useCallback(async () => {
    ...
    try {
      let nextRows: Row[];
      if (requestedTable === "customers") {
        nextRows = (await client.listCustomers()).customers;
      } else if (requestedTable === "products") {
        nextRows = (await client.listProducts()).products;
      } else {
        nextRows = (await client.listOrders()).orders;
      }
      if (requestId !== loadRequestId.current || requestedTable !== selectedTable.current) return;
      setRows(nextRows);
      setError("");
    } catch (cause) {
      if (requestId !== loadRequestId.current || requestedTable !== selectedTable.current) return;
